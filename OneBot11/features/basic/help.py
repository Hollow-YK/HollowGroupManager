...
class HelpModule:
    ...
    _CMD_FORMAT = {
        ...
        "approval": "{w}{cmd} <on|off|status|config>",
        "verify":  "{w}{cmd} <on|off|status|config|block|question>",
    }
    ...
    _CMD_EXAMPLES = {
        ...
        "approval": ["{w}{cmd} on", "{w}{cmd} config comment_regex 答案"],
        "verify":  ["{w}{cmd} config total_max_errors 10", "{w}{cmd} block add ALL"],
    }
    ...
    _CMD_DETAIL = {
        ...
        "approval":  ["> {w}{cmd} <on|off|status>              群开关",
                    "> {w}{cmd} config [key] [value]        统一配置管理",
                    "- 无参数  列出所有配置字段",
                    "- 单参数  查看字段值",
                    "- 多参数  设置字段值",
                    "= 配置 key（支持 snake_case / camelCase）",
                    "  comment_regex    匹配正则（空=不校验直接同意）",
                    "  reject_reason    拒绝原因文本",
                    "  on_mismatch      不匹配行为: reject=拒绝, ignore=忽略",
                    "  welcome_text     入群欢迎消息（空=不发送）",
                    "- 入群申请 comment 匹配正则 → 同意",
                    "~ {w}{cmd} config comment_regex 答案  /  {w}{cmd} config welcome_text 欢迎{@新成员}"],
        "verify":    [
                    "= 群开关",
                    "> {w}{cmd} on|off         开启/关闭本群验证（需群管理）",
                    "> {w}{cmd} status          查看状态：开关/方案概览/超时",
                    "",
                    "= 方案设置 (config) — 统一入口，直接用字段名",
                    "> {w}{cmd} config           列出所有配置字段",
                    "> {w}{cmd} config <key>     查看单个字段值",
                    "> {w}{cmd} config <key> <value>  设置字段值",
                    "- 配置 key（支持 snake_case / camelCase）:",
                    "  welcome_text            欢迎消息模板",
                    "  total_max_errors        总最多出错次数（-1=不限制）",
                    "  include_retry_in_total  重试错误是否计入总出错数（true/false）",
                    "  timeout_seconds         非Bot审批成员验证超时（0=不限）",
                    "  bot_approved_timeout    Bot审批成员验证超时（0=不限）",
                    "~ {w}{cmd} config total_max_errors 10",
                    "~ {w}{cmd} config timeout_seconds 300",
                    "",
                    "= 考核块 (block) — 按列表顺序执行",
