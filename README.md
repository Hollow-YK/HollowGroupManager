...
> 1.0.5 及以前的版本，两版共享相同的指令集、数据格式、权限模型和多配置架构。**数据文件可互换。**
>
> 从 1.0.6-beta.1 版本开始，QFun 版本功能更新会更慢甚至不再支持。
>
...
| `/admin` | 0 | `[唤醒词]admin [配置] <QQ用户> [等级]` | 权限管理（仅超管） |
| `/punish` | 1 | `[唤醒词]punish [配置] <QQ用户> <方式> [内容] <原因>` | 处罚成员 |
| `/revokepunish` | 1 | `[唤醒词]revokepunish [配置] <记录ID> [撤销原因]` | 撤销处罚 |
| `/history` | 1 | `[唤醒词]history [配置] [QQ用户] [-i]` | 查询记录 |
| `/verify` | 0 | `[唤醒词]verify <子命令>` | 进群答题验证（仅超管） |
| `/approval` | 0 | `[唤醒词]approval <子命令>` | 加群审批管理（仅超管） |
